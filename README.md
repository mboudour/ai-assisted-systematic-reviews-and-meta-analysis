...
The manuscript's measured empirical spine is deliberately narrow. In the prospective screening experiment, **190 of 200 records (95.00%)** retained the same label across all three calls, with **13 changed adjacent transitions among 400 comparisons**. Restricting the evaluator denominator changes the displayed CORRECT share from **58.39% to 98.52%**. Deterministic normalization changes design-weighted all-non-null categorical agreement from **21.58% to 46.13%**. Ten historical schemas yield **927 complete estimate-and-interval rows**, but none records both the study-level linkage and variance information needed to establish synthesis readiness. The design-weighted probability of changing null status is **1.92% (0.60%–4.34%)**; null states are therefore described as largely, but not perfectly, stable. The disclosure-safe [`evaluator_verdict_cube.csv`](results/tables/evaluator_verdict_cube.csv) recomputes all evaluator totals by case, field type, null status, and verdict.
