...
The final package was then rescanned with special treatment for the verbatim JSON Lines amendments. No author name, institution, email address, absolute path, URL, repository identifier, DOI, SSRN identifier, Zenodo identifier, or arXiv identifier appears in `config/amendments.jsonl`. The artifact builder and tests now include `.jsonl` in identity scanning. The supplement's model-conflict table drops the uncheckable prior-report section number, and the screening methods state that the deterministic 200-item subset contained 48.00% historical INCLUDE records versus 20.39% in the full archive.

## Final editor disclosure and ACM authorship-policy check

**Date:** 20 September 2026

The public SSRN abstract was revised and independently verified to display the screening-model correction and a 20 September 2026 revision date. The report PDF was not replaced. The identifying disclosure was removed from the reviewer workflow and rewritten as a separate confidential editor-only comment. It states what the new paper reuses, what it adds, why its lessons generalize to JDIQ's provenance, information-extraction, and LLM-output-quality scope, and that neither the manuscript nor the public report is under review elsewhere.

ACM's current authorship policy was checked directly. Writing assistance alone no longer requires disclosure, but AI used in research design, coding, analysis, testing, validation, reproducibility artifacts, or figures must be described in the Methods. The core manuscript now discloses Manus's assistance with post-outcome study redesign, analysis-code development and refactoring, automated-test construction, figure generation, and drafting/editing. It states that all results were regenerated from version-controlled scripts and tested against retained aggregates, that no generative-AI output served as an external truth label, and that the author accepts responsibility. The disclosure was fitted without exceeding the 10-page limit.
