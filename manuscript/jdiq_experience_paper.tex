...
LLM-assisted workflows turn unstructured text into decisions and structured records. Evidence synthesis connects retrieval, deduplication, screening, extraction, evaluation, and aggregation. Defects at one stage constrain later claims. High extraction agreement is uninterpretable without its denominator and abstention policy; a populated effect-size column does not establish a valid meta-analysis dataset.

Accuracy ordinarily requires external references. Here, independent human screening, source-level extraction, and adjudication were unavailable. This precludes factual-accuracy and evaluator-validity estimates, but not direct audits of completeness, provenance, operational observability, repeatability, denominator sensitivity, and downstream fitness.
...
\paragraph{Generative-AI use.} Manus, a general-purpose generative-AI system, assisted with post-outcome study redesign, analysis-code development and refactoring, automated-test construction, figure generation, and drafting and editing. All results were regenerated from version-controlled scripts and tested against retained aggregates; no generative-AI output served as an external truth label. The author accepts responsibility for all code, analyses, citations, and claims.
...
The screening frame first selected up to 30 records within each case-by-historical-label stratum by a seeded SHA-256 rank. The experiment then ranked those 1,192 frame records by a second seeded SHA-256 hash of the sample identifier and selected the first 200, independent of file order. The resulting subset covered all 20 cases and contained 96 historical INCLUDE (48.00\%) and 104 historical EXCLUDE records, enriching INCLUDE relative to its 20.39\% archive prevalence. Because the second stage was deterministic rather than a probability sample, we report the realized stability proportion and complete transition counts without a population claim.
