...
Research on LLM evaluation distinguishes lexical identity from semantic equivalence and documents limitations of LLM-as-judge designs \cite{kamalloo2023qa,zheng2023judge}. Abstention and insufficient-information states also change the target estimand: conditional performance among answered items is not the same as performance across all attempted items \cite{wen2025abstention}. Repeated calls can vary even when clients request low-temperature or nominally deterministic settings \cite{pagnamenta2025nondeterminism}. Our contribution is not another semantic metric. It is a controlled decomposition of exact agreement, deterministic normalization, token-set equality, and null-state transitions on the same frozen inputs.

Evidence-synthesis guidance emphasizes transparent search reporting and human accountability \cite{page2021prisma,rethlefsen2021prismas,flemyng2025ai}. Empirical studies show both the potential and limitations of LLM screening and extraction \cite{li2024screening,mahmoudi2025extraction,clark2025genai,lieberum2025llm}. Because this archive has no independent reference labels, we do not enter that accuracy literature as a competing benchmark. We instead examine the measurement and infrastructure conditions required before a benchmark claim would be meaningful.
...
Six cases reached their configured retrieval limits. Historical source-reported hit totals were not saved, so corpus completeness cannot be reconstructed. The archive retains only post-deduplication rows and not the removed record pairs, preventing estimation of false merges. Metadata completeness among retained records was 99.99\% for title, 98.95\% for abstract, 86.07\% for DOI, and 98.99\% for year. Language and source-native identifiers were not collected and are therefore unmeasured rather than zero. These facts characterize the corpus but are not among the three primary effects.
...
An initial protocol proposed human-referenced screening and extraction validation, evaluator error injection, meta-analysis, and calibrated propagation. After inspection of the archive and the later repeatability results, those branches were removed from the current paper because no external reference standard was available. The revised question asks which data-quality properties remain observable and identifiable without external truth labels, and how denominator rules, output representation, provenance gaps, failure-state encoding, and schema design affect those properties. The amendment was made after outcome inspection and was not preregistered. The original protocol and append-only amendments are included in the online supplement.
...
\addlinespace
Downstream-first schema design & Ten schemas yield 927 complete estimate-and-interval rows, but 0/20 contain a study-level linkage identifier, dedicated effect-measure label, variance/standard error, or dependence identifier. & Numeric completeness does not establish synthesis readiness. & A valid pooled effect or calibrated propagation result. \\
\bottomrule
\end{tabularx}
\end{table*}
...
\subsection{Null states are largely stable, while categorical exact agreement is representation-sensitive}
...
Table~\ref{tab:repeatability} reports all-non-null agreement. For categorical fields, deterministic normalization changes the weighted exact estimate from 21.58\% to 46.13\%. Token-set equality yields 47.79\%, while 53.35\% meet the minimum pairwise lexical-similarity threshold of 0.80. The case-cluster bootstrap interval for normalized categorical exact agreement is 34.76\%--56.34\%. Numeric fields are much more stable under all four representations. Their normalized exact estimate is 88.55\% (81.18\%--95.96\%). The residual categorical variation may include both harmless phrasing differences and substantive changes; without a semantic reference, those components cannot be separated.
...
A strict synthesis gate therefore admits no case from the archived fields alone. This result does not show that the extracted numbers are false. It shows that the data model did not record enough information to establish a common estimand, independent-study set, and uncertainty structure. The downstream model failed before statistical estimation became scientifically meaningful.
...
These results support a practical rule: data-quality controls must be designed around the claims the downstream workflow is expected to sustain. Denominator ledgers, typed event records, versioned execution provenance, representation-aware agreement, and analysis-driven schemas do not replace accuracy validation. They determine whether accuracy, reliability, and synthesis can be measured at all.

\section*{Data and Code Availability}

A double-anonymous review artifact is supplied as online supplementary material. It contains the derived tables, figure-generation code, repeatability and schema analyses, dependency specification, checksums, and automated tests needed to reproduce the reported results. It excludes raw bibliographic text, old manuscripts, Git history, author-identifying metadata, and the preserved historical repository snapshot.
