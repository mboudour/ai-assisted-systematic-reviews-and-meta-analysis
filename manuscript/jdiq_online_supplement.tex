...
The historical summaries report 95,292 records before within-case deduplication and 94,522 retained records, implying 770 reported removals (0.81\% of the pre-deduplication total). Table~\ref{tab:limits} reports every configured limit that was reached or exceeded. Case 5 exceeds its nominal limit because the OpenAlex loop appends complete 200-record pages and checks the stopping condition only before requesting the next page. The original source-reported totals were not retained, so no case can be certified complete retrospectively. Current source counts were obtained for 16 cases, but these are temporal diagnostics rather than reconstructions because index content and query semantics can change.

\begin{table}[h]
\caption{Cases that reached or exceeded a configured retrieval limit.}
\label{tab:limits}
\begin{tabular}{rlrrr}
\toprule
\textbf{Case} & \textbf{Source} & \textbf{Limit} & \textbf{Pre-dedup} & \textbf{Retained} \\
\midrule
3 & OpenAlex & 6,000 & 6,000 & 5,945 \\
5 & OpenAlex & 4,500 & 4,600 & 4,447 \\
8 & OpenAlex & 8,000 & 8,000 & 7,708 \\
11 & OpenAlex & 5,000 & 5,000 & 4,923 \\
15 & OpenAlex & 6,000 & 6,000 & 5,888 \\
20 & OpenAlex & 10,000 & 10,000 & 9,921 \\
\bottomrule
\end{tabular}
\end{table}
...
The screening frame first selected up to 30 records per case-by-historical-label stratum by the master-seed SHA-256 rank. It contained 1,192 records. The repeatability runner ranked the frame by \texttt{SHA256(seed | screening | sample\_id)} and selected the first 200, so selection was reproducible and independent of file order but was not a second probability sample. The subset covered all 20 cases and contained 96 historical INCLUDE (48.00\%) and 104 historical EXCLUDE records. It therefore enriched INCLUDE relative to its 20.39\% prevalence among all 94,522 archived screening decisions.

Of the 200 screening records, 190 had identical labels across all three calls, yielding 95.00\% exact three-call stability in the realized subset. Across the 400 adjacent repeat comparisons, 13 changed label: seven INCLUDE-to-EXCLUDE and six EXCLUDE-to-INCLUDE. The remaining comparisons comprised 145 INCLUDE-to-INCLUDE and 242 EXCLUDE-to-EXCLUDE transitions. These results measure prospective same-request stability, not screening accuracy or historical-model repeatability.
...
The case-cluster bootstrap interval for normalized exact agreement is 34.76\%--56.34\% for categorical items and 81.18\%--95.96\% for numeric items. Thus, numeric normalized exact agreement still has an 11.45\% design-weighted complement. The lexical thresholds are not validated semantic-equivalence cutoffs. They are included to show how conclusions vary under declared representation rules.
...
Ten schemas produce 927 complete estimate-and-interval rows. Seven cases pass a naive five-row numeric threshold and five pass a ten-row threshold. However, 0/20 schemas jointly contain a study-level linkage identifier, a dedicated effect-measure label, variance or standard error, and a dependence identifier. This is a schema-only coverage result; it has no human-verification criterion and does not claim that any extracted value is wrong.
...
The anonymous review artifact contains compact derived inputs, analysis scripts, figure-generation code, declared dependencies, automated tests, and the two append-only amendments. In particular, \path{evaluator_verdict_cube.csv} reports counts by case, field type, null status, and verdict without source text. This table allows the evaluator totals and null cross-tabulation to be recomputed. The artifact also contains the 200-item screening manifest, all 600 screening outputs reduced to labels, and the screening summary. The principal commands are:
...
The build is offline. It does not issue bibliographic queries or model calls. The artifact manifest records SHA-256 hashes for distributed files. Automated tests cover the evaluator cube, screening stability, consequence matrix, normalized extraction repeatability, figure generation, and artifact integrity.
